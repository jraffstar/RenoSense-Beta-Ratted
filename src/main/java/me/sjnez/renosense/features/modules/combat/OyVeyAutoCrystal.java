...
public class OyVeyAutoCrystal extends Module {
    ...
    private final me.sjnez.renosense.util.Timer preditTimer = new me.sjnez.renosense.util.Timer();
    private final me.sjnez.renosense.util.Timer manualTimer = new Timer();
    ...
    private void rotateToPos(BlockPos pos) {
        if (this.rotate.getValue().booleanValue()) {
            float[] angle = MathUtil.calcAngle(OyVeyAutoCrystal.mc.player.getPositionEyes(Util.mc.getRenderPartialTicks()), new Vec3d((float) pos.getX() + 0.5f, (float) pos.getY() - 0.5f, (float) pos.getZ() + 0.5f));
            this.yaw = angle[0];
            this.pitch = angle[1];
            this.rotating = true;
        }
    ...
    public void onCrystal() {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue().longValue()) && this.place.getValue()) {
            ...
            if (switchmode.getValue() == SwitchMode.Silent) {
                if (slot != -1) {
                    if (Util.mc.player.isHandActive() && this.silentSwitch.getValue()) {
                        hand = Util.mc.player.getActiveHand();
                    }
                    Util.mc.player.connection.sendPacket(new CPacketHeldItemChange(slot));
                }
            }
