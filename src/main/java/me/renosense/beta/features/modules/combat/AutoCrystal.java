...
import me.renosense.beta.event.events.ClientEvent;
import me.renosense.beta.event.events.PacketEvent;
import me.renosense.beta.event.events.Render3DEvent;
import me.renosense.beta.event.events.UpdateWalkingPlayerEvent;
...
public class AutoCrystal extends Module {
    ...
    public final me.renosense.beta.util.Timer threadTimer = new me.renosense.beta.util.Timer();
    ...
    private final me.renosense.beta.util.Timer switchTimer = new me.renosense.beta.util.Timer();
    private final me.renosense.beta.util.Timer manualTimer = new me.renosense.beta.util.Timer();
    private final me.renosense.beta.util.Timer breakTimer = new me.renosense.beta.util.Timer();
    private final me.renosense.beta.util.Timer placeTimer = new me.renosense.beta.util.Timer();
    private final me.renosense.beta.util.Timer syncTimer = new me.renosense.beta.util.Timer();
    private final me.renosense.beta.util.Timer predictTimer = new me.renosense.beta.util.Timer();
    private final me.renosense.beta.util.Timer renderTimer = new me.renosense.beta.util.Timer();
    private final AtomicBoolean shouldInterrupt = new AtomicBoolean(false);
    private final me.renosense.beta.util.Timer syncroTimer = new me.renosense.beta.util.Timer();
    private final Map<EntityPlayer, me.renosense.beta.util.Timer> totemPops = new ConcurrentHashMap<EntityPlayer, me.renosense.beta.util.Timer>();
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (!this.justRender.getValue().booleanValue() && this.switchTimer.passedMs(this.switchCooldown.getValue().intValue()) && this.explode.getValue().booleanValue() && this.instant.getValue().booleanValue() && event.getPacket() instanceof SPacketSpawnObject && (this.syncedCrystalPos == null || !this.syncedFeetPlace.getValue().booleanValue() || this.damageSync.getValue() == DamageSync.NONE)) {
            ...
            if (packet2.getType() == 51 && AutoCrystal.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + (double) this.predictOffset.getValue().floatValue() <= MathUtill.square(this.breakRange.getValue().floatValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue().intValue()) || this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue().intValue()))) {
                ...
                if (this.predictFriendDmg.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtill.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !RenoSense.friendManager.isFriend(friend) || !((double) DamageUtill.calculateDamage(pos, friend) > (double) EntityUtill.getHealth(friend) + 0.5))
                            continue;
                        return;
                    }
                }
            ...
            if (packet5.getOpCode() == 35 && packet5.getEntity(AutoCrystal.mc.world) instanceof EntityPlayer) {
                this.totemPops.put((EntityPlayer) packet5.getEntity(AutoCrystal.mc.world), new me.renosense.beta.util.Timer().reset());
            }
        ...
        for (Entity entity : AutoCrystal.mc.world.loadedEntityList) {
            ...
            if ((double) selfDamage + 0.5 < (double) EntityUtill.getHealth(AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue().floatValue()) {
                ...
                for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    ...
                    if (this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL || !RenoSense.friendManager.isFriend(player.getName()) || !((double) (damage = DamageUtill.calculateDamage(entity, player)) > (double) EntityUtill.getHealth(player) + 0.5))
                        continue;
                    ...
                    if (syncflag || this.currentDamage - (double) damageOffset > this.lastDamage || this.syncTimer.passedMs(this.damageSyncTime.getValue().intValue()) || this.damageSync.getValue() == DamageSync.NONE) {
                        ...
                        if (this.switchItem()) {
                            ...
                            if (this.foundDoublePop) {
                                this.totemPops.put(target, new me.renosense.beta.util.Timer().reset());
                            }
        ...
        for (BlockPos pos : BlockUtill.possiblePlacePositions(this.placeRange.getValue().floatValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            ...
            if (targettedPlayer != null) {
                ...
                if (this.calcEvenIfNoDamage.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.ALL || this.antiFriendPop.getValue() == AntiFriendPop.PLACE)) {
                    ...
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        float friendDamage;
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtill.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !RenoSense.friendManager.isFriend(friend) || !((double) (friendDamage = DamageUtill.calculateDamage(pos, friend)) > (double) EntityUtill.getHealth(friend) + 0.5))
                            continue;
                        friendPop = true;
                        break;
                    }
    ...
    private void rotateTo(Entity entity) {
        switch (this.rotate.getValue()) {
            ...
            case BREAK:
            case ALL: {
                float[] angle = MathUtill.calcAngle(AutoCrystal.mc.player.getPositionEyes(Util.mc.getRenderPartialTicks()), entity.getPositionVector());
    ...
    private void rotateToPos(BlockPos pos) {
        switch (this.rotate.getValue()) {
            ...
            case PLACE:
            case ALL: {
                float[] angle = MathUtill.calcAngle(AutoCrystal.mc.player.getPositionEyes(Util.mc.getRenderPartialTicks()), new Vec3d((float) pos.getX() + 0.5f, (float) pos.getY() - 0.5f, (float) pos.getZ() + 0.5f));
                if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE) {
                    RenoSense.rotationManager.setPlayerRotations(angle[0], angle[1]);
                    break;
                }
