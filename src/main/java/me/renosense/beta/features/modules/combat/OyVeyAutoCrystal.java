...
public class OyVeyAutoCrystal extends Module {
    ...
    private final me.renosense.beta.util.Timer preditTimer = new me.renosense.beta.util.Timer();
    ...
    private void rotateTo(Entity entity) {
        if (this.rotate.getValue().booleanValue()) {
            float[] angle = MathUtil.calcAngle(OyVeyAutoCrystal.mc.player.getPositionEyes(me.renosense.beta.util.Util.mc.getRenderPartialTicks()), entity.getPositionVector());
            this.yaw = angle[0];
            this.pitch = angle[1];
            this.rotating = true;
        }
    ...
    private void rotateToPos(BlockPos pos) {
        if (this.rotate.getValue().booleanValue()) {
            float[] angle = MathUtil.calcAngle(OyVeyAutoCrystal.mc.player.getPositionEyes(me.renosense.beta.util.Util.mc.getRenderPartialTicks()), new Vec3d((float) pos.getX() + 0.5f, (float) pos.getY() - 0.5f, (float) pos.getZ() + 0.5f));
            this.yaw = angle[0];
            this.pitch = angle[1];
            this.rotating = true;
        }
    ...
    public void onCrystal() {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue().longValue()) && this.place.getValue()) {
            ...
            int old = me.renosense.beta.util.Util.mc.player.inventory.currentItem;
            EnumHand hand = null;
            if (switchmode.getValue() == SwitchMode.Silent) {
                if (slot != -1) {
                    if (me.renosense.beta.util.Util.mc.player.isHandActive() && this.silentSwitch.getValue()) {
                        hand = me.renosense.beta.util.Util.mc.player.getActiveHand();
                    }
                    me.renosense.beta.util.Util.mc.player.connection.sendPacket(new CPacketHeldItemChange(slot));
                }
            }
            ...
            if (switchmode.getValue() == SwitchMode.Silent) {
                if (slot != -1) {
                    if (this.silentSwitch.getValue() && hand != null) {
                        me.renosense.beta.util.Util.mc.player.setActiveHand(hand);
                    }
                    Util.mc.player.connection.sendPacket(new CPacketHeldItemChange(old));
    ...
    EntityPlayer getTarget() {
        ...
        for (EntityPlayer entity : OyVeyAutoCrystal.mc.world.playerEntities) {
            if (OyVeyAutoCrystal.mc.player == null || OyVeyAutoCrystal.mc.player.isDead || entity.isDead || entity == OyVeyAutoCrystal.mc.player || RenoSense.friendManager.isFriend(entity.getName()) || entity.getDistance(OyVeyAutoCrystal.mc.player) > 12.0f)
                continue;
            this.armorTarget = false;
