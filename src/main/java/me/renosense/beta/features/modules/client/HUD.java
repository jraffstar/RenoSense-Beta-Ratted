...
public class HUD extends Module {
    ...
    private final me.renosense.beta.util.Timer timer = new Timer();
    ...
    public void onRender2D(Render2DEvent event) {
            ...
            if (this.renderingUp.getValue().booleanValue()) {
                if (this.renderingMode.getValue() == RenderingMode.ABC) {
                    for (int k = 0; k < RenoSense.moduleManager.sortedModulesABC.size(); k++) {
                        String str = RenoSense.moduleManager.sortedModulesABC.get(k);
        ...
        if (this.renderingUp.getValue().booleanValue()) {
            ...
            if (potions.getValue().booleanValue()) {
                ...
                for (PotionEffect potionEffect : effects) {
                    String str = RenoSense.potionManager.getColoredPotionString(potionEffect);
                    i += 10;
            ...
            if (this.server.getValue()) {
                String sText = grayString + "Server " + ChatFormatting.WHITE + (Util.mc.isSingleplayer() ? "SinglePlayer" : Util.mc.getCurrentServerData().serverIP);
                i += 10;
            ...
            if (this.speed.getValue().booleanValue()) {
                String str = grayString + "Speed " + ChatFormatting.WHITE + RenoSense.speedManager.getSpeedKpH() + " km/h";
                i += 10;
            ...
            if (this.tps.getValue().booleanValue()) {
                String str = grayString + "TPS " + ChatFormatting.WHITE + RenoSense.serverManager.getTPS();
                i += 10;
                this.renderer.drawString(str, (width - this.renderer.getStringWidth(str) - 2), (height - 2 - i), (ClickGui.getInstance()).rainbow.getValue().booleanValue() ? (((ClickGui.getInstance()).rainbowModeA.getValue() == ClickGui.rainbowModeArray.Up) ? ColorUtil.rainbow(counter1[0] * (ClickGui.getInstance()).rainbowHue.getValue().intValue()).getRGB() : ColorUtil.rainbow((ClickGui.getInstance()).rainbowHue.getValue().intValue()).getRGB()) : this.color, true);
                counter1[0] = counter1[0] + 1;
            }
            ...
            String sText = grayString + "Server " + ChatFormatting.WHITE + (Util.mc.isSingleplayer() ? "SinglePlayer" : Util.mc.getCurrentServerData().serverIP);
            String str1 = grayString + "Ping " + ChatFormatting.WHITE + RenoSense.serverManager.getPing() + (this.ms.getValue() ? "ms" : "");
        ...
        String direction = this.direction.getValue().booleanValue() ? RenoSense.rotationManager.getDirection4D(false) : "";
    ...
    public void onLoad() {
        RenoSense.commandManager.setClientMessage(getCommandMessage());
    }
