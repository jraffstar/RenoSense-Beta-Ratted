package me.renosense.beta.util;
...
import static me.renosense.beta.util.HoleFillUtil.faceVectorPacketInstant;
import static me.renosense.beta.util.HoleFillUtil.processRightClickBlock;
...
public class BlockUtill
    ...
    public static List<BlockPos> getBlockSphere(float breakRange, Class clazz) {
        ...
        positions.addAll(BlockUtill.getSphere(EntityUtil.getPlayerPos(mc.player), breakRange, (int) breakRange, false, true, 0).stream().filter(pos -> clazz.isInstance(mc.world.getBlockState(pos).getBlock())).collect(Collectors.toList()));
        return positions;
    }
    ...
    public static List<BlockPos> possiblePlacePositions(float placeRange) {
        NonNullList positions = NonNullList.create();
        positions.addAll(BlockUtill.getSphere(EntityUtil.getPlayerPos(mc.player), placeRange, (int) placeRange, false, true, 0).stream().filter(BlockUtill::canPlaceCrystal).collect(Collectors.toList()));
        return positions;
    }
    ...
    public static List<BlockPos> possiblePlacePositions(float placeRange, boolean specialEntityCheck, boolean oneDot15) {
        NonNullList positions = NonNullList.create();
        positions.addAll(BlockUtill.getSphere(EntityUtil.getPlayerPos(mc.player), placeRange, (int) placeRange, false, true, 0).stream().filter(pos -> BlockUtill.canPlaceCrystal(pos, specialEntityCheck, oneDot15)).collect(Collectors.toList()));
        return positions;
    }
